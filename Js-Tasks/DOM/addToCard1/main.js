...
const quickviewSec = document.querySelector('#quick-view')
const quickviewBasket = document.querySelector('#quick-basket')
...
function render(list = datas) {
    ...
    list.slice(start, end).forEach(item => {
        ...
        <div class="card">
            <div>
                <div class="back-img">
                    <img src="${item.img}" onclick='quickView("${item.id}")'  onerror="this.src='./download.png'">
                </div>
                <!-- card-main -->
                <div class="main">
                    <h3>${item.name}</h3>
                    ...
                    <div class="main-bottom">
                        <div>
                            <button onclick='addToCart("${item.id}")' class="cursor-pointer bg-violet-200 p-[7px_15px]">ADD</button>
                        </div>
                    </div>
                </div>
            </div>
...
function generatePagination() {
    let totalPages = globalData.length / 12
...
// paginationsFunction
function getPagination(page_num = 1) {
    start = (page_num - 1) * 12
    end = page_num * 12
    render(globalData)
    window.scrollTo({
...
searchInput.addEventListener('input', () => {
    ...
    const filteredList = datas.filter(item => item.name.toLowerCase().includes(inpVal) || item.dsc.toLowerCase().includes(inpVal))
    render(filteredList)
...
basketBtn.addEventListener('click', () => {
    quickviewBasket.classList.remove('hidden')
    cardListRender()
...
quickviewBasket.addEventListener('click', (e) => {
    if (e.target === quickviewBasket) {
        quickviewBasket.classList.add("hidden");
    }
})
